    ...
    <el-row v-if="quanxian==60||quanxian==28" class="send_file">
      <div class="fasong">
        ...
        <el-form v-if="xiugai" class="renyuan" ref="form" :model="xiugaiform" label-width="80px">
          <el-input v-model="userid" placeholder="请输入用户账号"></el-input><el-button class="anniu" type="primary" @click="chaxun">查询</el-button>
      ...
      <!-- 删除人员 -->
        <tree-transfer v-if="shanchu" class="eltree" :title="shanchutitle" :from_data='shanchufrom' :to_data='shanchuto' :defaultProps="{label:'label'}" @addBtn='shanchuadd' @removeBtn='shanchuremove' :mode='mode' width="50%" height="500px" filter>
        </tree-transfer>
      ...
      <div v-if="xinjian">
        ...
        <div>
          <tree-transfer class="eltree" :title="xinjiantitle" :from_data='xinjianfrom' :to_data='xinjianto' :defaultProps="{label:'label'}" @addBtn='xinjianadd' @removeBtn='xinjianremove' :mode='mode' width="50%" height="500px" filter>
          </tree-transfer>
        </div>
      </div>

      <!-- 批量导入 -->
      <el-upload v-if="piliang" class="upload" ref="upload" drag :auto-upload="false" :on-success="onsuccess" :action="piliangdaoru" :data="upload">
...
export default {
  ...
  methods: {
    ...
    chaxun(){
      ...
      API.getUser({'token':localStorage.getItem('token'),'userid':this.userid}).then(({
        data
      }) => {
        if(data.MSG=='NO'){
          this.$message({
    ...
    querenxiugai(){
      if(this.xiugaiform.id.length==0||this.xiugaiform.pwd.length==0||this.xiugaiform.name.length==0||this.xiugaiform.group.length==0){
        this.$message({
            showClose: true,
      ...
      API.setUserProfile(this.xiugaiform).then(({
        data
      }) => {
          this.$message.success({
              showClose: true,
    ...
    querenxinzeng(){
      if(this.xinzengform.id.length==0||this.xinzengform.pwd.length==0||this.xinzengform.name.length==0||this.xinzengform.group.length==0){
        this.$message({
            showClose: true,
            message: '账号/密码/姓名/所在部门不能为空',
            duration: 2000
      ...
      API.xinzengrenyuan({'token':localStorage.getItem('token'),'newuser':this.xinzengform})
        .then(({
          data
        }) => {
          if(data.MSG=='YES'){
    ...
    querenshanchu(){
      ...
      this.$confirm('确认删除？请谨慎操作', '提示', {
          ...
          type: 'warning'
            ...
            API.shanchurenyuan({'token':localStorage.getItem('token'),'shanchuto':this.shanchuto})
              .then(({
                data
              }) => {
                if(data.MSG=='YES'){
