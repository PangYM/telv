  ...
  <div class="oa_main">
    ...
    <el-row v-if="quanxian==60||quanxian==28" class="send_file">
      <div class="fasong">
        ...
        <!-- 登录页 -->
        <el-table class="dengluye" v-if="dengluye"
        :data="dengluyelist"
        border
        stripe>
        <el-table-column
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <el-button size="mini" style="margin-bottom:20px;" v-if="scope.row.show==0" @click="xianshireirong(scope.$index, scope.row,1)">显示内容</el-button>
            <el-button size="mini" style="margin-bottom:20px;" v-else @click="xianshireirong(scope.$index, scope.row,0)">隐藏内容</el-button>
            <vue-editor v-if="scope.row.show==1" style="height:1000px;"
        ...
        <el-table-column
          ...
          width="60">
          <template slot-scope="scope">
            <el-button size="small" icon="el-icon-plus" @click="handleAddTask0()" v-if="scope.$index == dengluyelist.length - 1&&dengluyelist.length < 5" :disabled="dengluyelist.length == 10 || !(scope.row.title && scope.row.content)"></el-button>
            <el-button size="small" icon="el-icon-minus" @click="handleRemoveTask0(scope.$index)" v-else></el-button>
          </template>
        </el-table-column>
      </el-table>

      <!-- 轮播图 -->
      <el-table class="dengluye" v-if="lunbotu"
        :data="lunbotulist"
        border
        stripe>
        <el-table-column
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <el-button size="mini" style="margin-bottom:20px;" v-if="scope.row.show==0" @click="xianshireirong1(scope.$index, scope.row,1)">显示内容</el-button>
            <el-button size="mini" style="margin-bottom:20px;" v-else @click="xianshireirong1(scope.$index, scope.row,0)">隐藏内容</el-button>
            <vue-editor v-if="scope.row.show==1" style="height:1000px;"
            useCustomImageHandler
            @imageAdded="handleImageAdded"
            v-model="scope.row.content">
        ...
        <el-table-column
          ...
          width="60">
          <template slot-scope="scope">
            <el-button size="small" icon="el-icon-plus" @click="handleAddTask1()" v-if="scope.$index == lunbotulist.length - 1&&lunbotulist.length < 5" :disabled="lunbotulist.length == 10 || !(scope.row.url && scope.row.content)"></el-button>
            <el-button size="small" icon="el-icon-minus" @click="handleRemoveTask1(scope.$index)" v-else></el-button>
          </template>
        </el-table-column>
      </el-table>

      <!-- 友情链接 -->
      <el-table class="youqing" v-if="youqing"
        :data="youqinglist"
        border
        stripe>
        <el-table-column
        ...
        <el-table-column
          ...
          width="60">
          <template slot-scope="scope">
            <el-button size="small" icon="el-icon-plus" @click="handleAddTask2" v-if="scope.$index == youqinglist.length - 1" :disabled="youqinglist.length == 10 || !(scope.row.title && scope.row.url)"></el-button>
            <el-button size="small" icon="el-icon-minus" @click="handleRemoveTask2(scope.$index)" v-else></el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    querenlunbotu(){
      API.setlunbotulist({'token':localStorage.getItem('token'),'lunbotulist':this.lunbotulist})
      .then(({
        data
      }) => {
        if(data.MSG=='YES'){
    ...
    querendengluye(){
      API.setdengluyelist({'token':localStorage.getItem('token'),'dengluyelist':this.dengluyelist})
      .then(({
        data
      }) => {
        if(data.MSG=='YES'){
    ...
    querenyouqing(){
      API.setyouqinglist({'token':localStorage.getItem('token'),'youqinglist':this.youqinglist})
      .then(({
        data
      }) => {
        if(data.MSG=='YES'){
